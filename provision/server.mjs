...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/whoami") {
      if (!caller) return send(res, 401, { error: "unrecognized caller — present a registered identity key" });
      return send(res, 200, await whoAmI(caller));
    }

    if (url.pathname === "/mcp") {
      if (req.method !== "POST") return send(res, 405, { error: "MCP is POST-only on this endpoint" });
      return handleMcp(req, res, await readBody(req), caller);
    }

    if (req.method === "GET" && url.pathname === "/api/health") {
      return send(res, 200, { ok: true, model: "Xenova/distilgpt2" });
    }

    if (req.method === "GET" && url.pathname === "/api/graphs") {
      return send(res, 200, { graphs: listProvisioned() });
    }

    const m = url.pathname.match(/^\/api\/graphs\/([a-z0-9-]+)$/);
    if (req.method === "GET" && m) {
      const g = listProvisioned().find((x) => x.slug === m[1]);
      return g ? send(res, 200, g) : send(res, 404, { error: "not provisioned" });
    }

    if (req.method === "POST" && url.pathname === "/api/provision") {
      const body = await readBody(req);
      if (!body.auto && !(body.description || "").trim())
        return send(res, 400, { error: "description is required (or set auto=true)" });
