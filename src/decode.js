/* Governed decoding — FUSE inside the active decode loop, per token.
   ...
   The rest of the runtime governs a distribution that arrives through a seam.
   This module closes that gap: it runs the token-by-token decode loop itself
   and applies FUSE at every single step, so a prohibited token is zeroed in
   the governed distribution the sampler draws from and can never be emitted,
   at any step. It also captures the real per-token cognitive trace (the raw
   and governed distributions, the entropy of each, and which tokens were
   forced to zero), then seals it so a third party recomputes the whole decode
   from the record alone.

   The model enters through `logitsFn(tokens, step) -> number[] over the
   vocabulary`. That is the one seam you point at a live model:
     - transformers.js / any local model: wrap next-token logits (see
       examples/run-igl-language-live.mjs).
     - a vendor that returns logprobs: exponentiate the logprobs into logits.
   The mechanism is identical whatever produces the logits.

   z′ = z + ln(wᵢ): multiplicative governance at the probability level is
   additive governance at the logit level. Prohibited tokens carry weight 0,
   which is -Infinity in log space — they are unreachable, not discouraged. */
...
/* A logits processor for ANY external decoder (transformers.js generate(), a
   custom sampler, etc.): prohibited tokens go to -Infinity so softmax gives
   them exactly zero mass and they can never be sampled. Permitted tokens keep
   their logit; the multiplicative steering is applied at the probability
   level inside governedDecode. This is the drop-in that puts governance
   inside a real model's decode. */
...
/* One governed decode step, exposed so a host loop can call it directly:
   apply FUSE to the raw distribution, enforce support restriction, check the
   graded token ceilings, and return the governed distribution plus the
   per-token record. */
...
/* Hash-chain the per-step governed digests into one decode digest, so the
   whole token-by-token trace is a single recomputable, tamper-evident object. */
...
/* Recompute the decode from the record: confirm support restriction on every
   governed step and the chained digests. This is the check a skeptic runs on
   the per-token trace without trusting the runtime that produced it. */
