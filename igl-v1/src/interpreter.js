...
import { IOSPlus, VOCAB, valueOf, effectiveDelegatedAuthority } from "./iosplus.js";
...
export class Interpreter {
  ...
  evalFuse(e, ctx) {
    ...
    if (e.under) {
      ...
      if (delegates) {
        // ADR 0001 item 3: a delegated FUSE runs at the target's DECLARED authority,
        // never its inherited elevation. Delegation reaches a scope, it does not
        // borrow the scope's inherited super-authority. See AUTHORITY_POLICY.
        const declared = declaredAuthorityOf(e.under, decls) ?? u.authority;
        const acting = effectiveDelegatedAuthority(declared, u.authority);
        ios.audit({ op: "FUSE_UNDER", target: u.id, delegated: true, targetResolved: u.authority, targetDeclared: declared, actingAuthority: acting, delegatedFrom: actor?.id ?? null });
        actor = { ...u, authority: acting, viaDelegation: true, delegatedFrom: actor?.id ?? null };
      } else {
        ios.audit({ op: "FUSE_UNDER", target: u.id, delegated: false, actingAuthority: u.authority });
        actor = u;
      }
    }
...
/* The DECLARED authority of a FUSE UNDER target, read from the program operand
   (pre-inheritance). Used by the delegation min-clamp so a delegated turn never
   claims the target's inherited elevation, only the level it was declared with. */
function declaredAuthorityOf(ref, decls) {
  const d = ref.kind === "IdUri"
    ? Object.values(decls.identities).find(x => x.id === ref.value)
    : decls.identities[ref.name];
  return d && typeof d.authority === "number" ? d.authority : undefined;
}
