...
- Authority law decided and encoded (ADR 0002): structural composition is
  MIN-intersection, so no arrangement of graph edges can ever raise authority, and
  explicit DELEGATE TO is the only escalation, clamped to min(declared, resolved).
  The law, the narrowing propagation, the chain minimum, the clamp, and the
  refusal path are all tested.
