...
- `interpreter.js` `execStatement` and `fuseDist`, for the fixed order and the FUSE
  math with its recomputable record.
- `determination.js` `determine`, `resolvePosition`, `entail`, `sealReceipt`,
  `verifyDetermination`, for the deterministic governance and the receipt chain.
- `sign.js` `signReceipt` and `verifyReceipt`, for the signature envelope.
- `d1.js` `persistReceipt`, for how a receipt lands in the real `audit_receipts` chain.
